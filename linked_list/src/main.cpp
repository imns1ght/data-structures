...
#define MOVE_SYNTAX_IMPLEMENTED
...
int main(void) {
	...
	// Unit #4: range constructor
	{
		...
		for (auto it = vec3.begin(); it != vec3.end(); ++it, ++it2) assert(*it == *it2);
...

	{
		...
		auto i{5};
		for (const auto& e : seq) assert(e == i--);
...
#ifndef IGNORE_THIS
	// This method has the same signature as the assign( InItr, InItr ) of InItr is an
	// integral type.
	...
	{
		std::cout << ">>> Unit teste #" << ++n_unit << ": assign(InItr first, InItr last).\n";
	...
	{
		std::cout << ">>> Unit teste #" << ++n_unit << ": assign(std::initializer_list<T> ilist).\n";

		// Initial vector.
		sc::List<int> seq{1, 2, 3, 4, 5};
		sc::List<int> seq2;

		seq2.assign({1, 2, 3, 4, 5});
		assert(seq == seq2);
		assert(seq2.size() == 5);
