...
struct KeyHash {
        std::size_t operator()(const Account::AcctKey &k_) const {
                std::string client_name = std::get<0>(k_);
                size_t hash_bank_num = std::get<1>(k_);
                size_t hash_agency_num = std::get<2>(k_);
                size_t hash_acc_num = std::get<3>(k_);
                size_t hash_client_name = 0;

                for (auto i = client_name.begin(); i < client_name.end(); i++) {
                        hash_client_name += *i;
                }

                // std::cout << "[DEBUG] (struct KeyHash)\n\n";
                // std::cout << "hash_client_name: " << hash_client_name << " (" << std::get<0>(k_)
                //           << ")" << std::endl;                                     // DEBUG
                // std::cout << "hash_bank_num: " << hash_bank_num << std::endl;      // DEBUG
                // std::cout << "hash_agency_num: " << hash_agency_num << std::endl;  // DEBUG
                // std::cout << "hash_acc_num: " << hash_acc_num << std::endl;        // DEBUG
                // std::cout << "xor: "
                //           << (hash_client_name xor hash_bank_num xor hash_agency_num xor
                //               hash_acc_num)
                //           << std::endl;  // DEBUG

                return (hash_client_name xor hash_bank_num xor hash_agency_num xor hash_acc_num);
        }
};
...
TEST_F(HTTest, InitialState) {
        EXPECT_TRUE(ht_accounts.empty());
        EXPECT_EQ(ht_accounts.size(), 0);
}
...
TEST_F(HTTest, InsertingData) {
        ...
        for (auto &e : m_accounts) {
                ...
                EXPECT_EQ(ht_accounts[e.getKey()], e);
        }
}
...
TEST_F(HTTest, OperatorSquareBraketsRHS) {
        ...
        for (auto &e : m_accounts) EXPECT_EQ(ht_accounts[e.getKey()], e);
}
...
TEST_F(HTTest, OperatorSquareBraketsLHS) {
        ...
        EXPECT_EQ(curr_size, ht_accounts.size());

        i = 10;
        // Check the table if the changes took place.
        for (auto &e : m_accounts) {
                auto x = ht_accounts[e.getKey()];
                EXPECT_EQ(x.m_balance, 100. + i);
                i += 10;
        }
}
...
TEST_F(HTTest, OperatorSquareBraketsLHS2) {
        ...
        for (const auto &w : {"this", "sentence", "is", "not", "a", "sentence", "this", "sentence",
                              "is", "a", "hoax"}) {
                ++word_map[w];
                ++expected[w];
        }

        EXPECT_EQ(expected.size(), word_map.size());

        // Compare the two dictionaries
        bool entered{false};
        for (const auto &pair : expected) {
                entered = true;
                EXPECT_EQ(pair.second, word_map[pair.first]);
        }
        EXPECT_TRUE(entered);
}
...
TEST_F(HTTest, AtRHS) {
        ...
        // Retrieve each element
        for (auto &e : m_accounts) EXPECT_EQ(ht_accounts.at(e.getKey()), e);
}
...
TEST_F(HTTest, AtLHS2) {
        ...
        for (const auto &pair : expected) {
                entered = true;
                EXPECT_EQ(pair.second * 10, word_map.at(pair.first));
        }
        EXPECT_TRUE(entered);
}
...
TEST_F(HTTest, CopyConstructor) {
        ...
        for (const auto &e : expected) {
                ...
                EXPECT_TRUE(result);
                EXPECT_EQ(e.second, data);
        }

        EXPECT_EQ(copy.size(), word_map.size());
        EXPECT_EQ(expected.size(), copy.size());
}
...
TEST_F(HTTest, ConstructorInitializer) {
        ...
        EXPECT_EQ(htables.size(), expected.size());
}
...
TEST_F(HTTest, AssignmentOperator) {
        ...
        EXPECT_EQ(htable_copy.size(), expected.size());
}
...
TEST_F(HTTest, InsertExisting) {
        ...
        for (const auto &e : expected) {
                int data;
                auto result = htable.retrieve(e.first, data);
                EXPECT_TRUE(result);        // We must found the key
                EXPECT_NE(e.second, data);  // BUT, the data must differ
        }
        ...
        for (const auto &e : expected) {
                ...
                EXPECT_FALSE(result);  // False in case the element is already in the table.
        }
...
TEST_F(HTTest, Retrieve) {
        ...
        std::map<char, int> unexpected{{'s', 27}, {'e', 3}, {'g', 1},
                                       {'q', 21}, {'i', 6}, {'j', 11}};
...
TEST_F(HTTest, EraseExisting) {
        ...
        for (const auto &e : expected) {
                int data = 0;
                auto result = htable.retrieve(e.first, data);
                EXPECT_FALSE(result);  // We must NOT found the key
                EXPECT_EQ(data, 0);
        }
...
TEST_F(HTTest, EraseNonExisting) {
        ...
        EXPECT_EQ(curr_size, htable.size());  // This must've not changed.
...
TEST_F(HTTest, Clear) {
        ...
        EXPECT_NE(curr_size, htable.size());
        EXPECT_TRUE(htable.empty());
        EXPECT_EQ(htable.size(), 0);
}
...
TEST_F(HTTest, Rehash) {
        ...
        for (const auto &e : set1) {
                auto result = htable.insert(e.first, e.second);
                EXPECT_TRUE(result);  // We must NOT found the key
        }
        EXPECT_EQ(set1.size(), htable.size());  // This must've not changed.
...
TEST_F(HTTest, Count) {
        ...
        std::map<int, std::string> set1{{11, "eleven"},
                                        {2 * 11, "twenty two"},
                                        {3 * 11, "thirty three"},
                                        {4 * 11, "fourty four"}};
        std::map<int, std::string> set2{
            {5, "five"}, {2 * 11 + 5, "twnety seven"}, {3 * 11 + 5, "thirty eight"}};
        ...
        for (const auto &e : set1) EXPECT_EQ(set1.size(), htable.count(e.first));
        for (const auto &e : set2) EXPECT_EQ(set2.size(), htable.count(e.first));
        for (const auto &e : set3) EXPECT_EQ(set3.size(), htable.count(e.first));
        // std::cout << "The table: \n" << htable << std::endl;
}
